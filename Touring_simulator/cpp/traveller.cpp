...
inline void Traveller::PrintID() const
{
    std::cout << id_ << std::endl;
}

inline const std::string &Traveller::get_ID() const
{
    return id_;
}

inline void Traveller::set_id(std::string name)
{
    id_ = name;
}

inline void Traveller::ShowPath() const
{
    touring_path_.Show();
}

inline const Path &Traveller::get_path() const
{
    return touring_path_;
}

Path Traveller::SchedulePath(const CityGraph &graph, Strategy s, Time t, Time limit)
{
  return SchedulePath(graph, travelling_plan_, s, t, limit);
}

inline void Traveller::set_path(Path &path)
{
  touring_path_ = path;
  position_pathnode_ = -1;
}
...
inline const std::vector<City_id> &Traveller::get_plan() const
{
    return travelling_plan_;
}

inline void Traveller::set_plan(const std::vector<City_id> &plan)
{
    travelling_plan_ = plan;
}

inline void Traveller::set_plan(std::initializer_list<City_id> il)
{
    travelling_plan_ = std::vector<City_id>(il);
}

inline void Traveller::append_plan(City_id city)
{
    travelling_plan_.push_back(city);
}

inline int Traveller::get_position() const
{
    return position_pathnode_;
}

inline int Traveller::get_left_hour() const
{
    return next_city_hour_left_;
}

inline TravellerState Traveller::get_state() const
{
    return state_;
}

inline void Traveller::set_strategy(Strategy strategy)
{
    strategy_ = strategy;
}
...
inline void Traveller::set_init_time(const Time &t)
{
    init_time_ = t;
}

inline const Time &Traveller::get_init_time() const
{
    return init_time_;
}
