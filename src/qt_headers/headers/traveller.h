...
public:
  ...
  const std::vector<City_id> &get_plan() const;

  void set_plan(const std::vector<City_id> &plan);
  void set_plan(std::initializer_list<City_id> il);
  // 保存当前旅客信息
  bool SaveData() const;

  bool LoadData(int cnt, const CityGraph &graph);

  void append_plan(City_id city);

  int get_position() const;

  int get_left_hour() const;

  void InitState(const CityGraph &graph);

  void UpdateState(const CityGraph &graph, Time now);

  TravellerState get_state() const;

  void set_strategy(Strategy strategy);
  bool set_strategy(int strategy);

  void set_init_time(const Time &t);
  const Time &get_init_time() const;
